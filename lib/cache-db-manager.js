...
define([
    ...
    var logger = common.logger.child('cache-db-manager');
    ...
    if (!window.localStorage || !window.indexedDB) {
        logger.error('cannot get localstore nore indexedDB factory from browser',
    ...
    var privates = {
        ...
        saveCatalog: function saveCaches(data) {
            ...
            window.localStorage.setItem(CACHE_DB_STORAGE_KEY, JSON.stringify(toWrite));
        },
            ...
            begin: function beginCacheTransaction(db, txName, resolve, reject) {
                ...
                if (typeof(resolve) === 'function') {
                    tx.oncomplete = function (event) {
                        ...
                        resolve(); // resolves undefined. need?
                    };
                }
                if (typeof(reject) === 'function') {
