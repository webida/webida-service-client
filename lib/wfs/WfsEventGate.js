...
define([
    ...
    var logger = common.logger.child('WfsEventGate');
    ...
    function WfsEventGate(session, wfsId) {
        ...
        session.on('workspace.watcher', this._onWatcherEventBound);
    }
    ...
    WfsEventGate.prototype = {
        ...
        stopListening: function stopListening() {
            if (this.session) {
                this.session.off('workspace.watcher', this._onWatcherEventBound);
        ...
        _onWatcherEvent : function _onWatcherEvent(eventData) {
            var wfsId = eventData.wfsId;
            var type = eventData.type;

            // { wfsId, type, path, stats } = data
            // pre-check - watcher's state change events will not be populated.
            if (wfsId !== this.wfsId) {
                return;
            }
            if (type === 'start' || type === 'stop' || type === 'ready') {
                logger.debug('ignores watcher state change events', eventData);
                return;
            }
            if (type === 'error') {
                logger.warn('server watcher has some error', eventData.error);
                return;
            }

            var wfsPath = eventData.wfsPath;
            var stats = eventData.stats;
            var holder = this.holders[wfsPath];
            if (!holder) {
                holder = this._addNewHolder(wfsPath);
            }
        ...
        _fireEvents: function _fireEvents(events,startedAt) {
            ...
            events.forEach(function(event) {
                ...
                myself.session.emit('workspace.wfs', myself.wfsId, event.type, event.path, wstats);
            });
