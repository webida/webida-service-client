...
define([
    ...
    var SUPPORTED_RESPONSE_TYPES = {
        ...
        blob: true,
        arraybuffer: true,
        json: true  // available only when response content-type is application/json
    };
    ...
    WfsMount.prototype = {
        ...
        readFile : function wfsReadFile(path, responseType, callback) {
            ...
            if (resultType && !SUPPORTED_RESPONSE_TYPES[resultType]) {
                var msg = 'unsupported response type ' + resultType;
                msg += ', should be one of ' + JSON.stringify(SUPPORTED_RESPONSE_TYPES);
            ...
            if (resultType && resultType !== 'text' && resultType !== 'json') {
                callbackWrapper.responseType = resultType;
            }
