...
define([
    ...
    var logger = common.logger.child('WfsOfflineCache');
    var wfsApi =  new common.api.WfsApi();
    var dbUtils = cacheDbManager.utils;
    var CACHE_TTL = 86400*1000;

    function WfsOfflineCache(workspace, sessionClient) {
        this.wfsId = workspace.id;
        this.sessionClient = sessionClient;
        this.db = null;
        this.dirForest = null;  // become truthy after init

        this._offlineCachePaths = workspace.offlineCachePaths || [];
        ...
        sessionClient.on('wfs', this._onWatcherEvent.bind(this));
        ...
        sessionClient.on('disconnect', this._onDisconnected.bind(this));
        sessionClient.on('connect', this._onConnected.bind(this));
    }
    ...
    WfsOfflineCache.prototype = {
        ...
        _onConnected: function _onConnected() {
            ...
            if (this.dirForest) {
                // send events via session, please.
                this._uploadLocalChanges();
            } else {
                if (!this._initPromise) {
                    this.init().then( function() {
                        logger.debug('initialized by session connect event');
                    });
                }
            }
        ...
        init: function init() {
            var myself = this;
            var initPromise = cacheDbManager.open(this.wfsId)
                .then( function(db) {
                    myself.db = db;
        ...
        // clean old items should be called by workspace api service
        // periodically (once in 10m~1h will do)
            ...
            return new Promise(function (resolve, reject) {
                ...
                this.readFromCache(wfsPath).then( function(item) {
                    ...
                    if (myself._shouldEmitWfsEvents) {
                        myself.sessionClient.emit('wfs', '_change', this.wfsId, item.stats);
                    }
                }).catch( function(err) {
        ...
        _buildForest: function _buildForest() {
            var forest = new WfsDirForest(this.wfsId, this._offlineCachePaths);
            var myself = this;
            return forest.build().then( function() {
                myself.dirForest = forest;
            });
        },
