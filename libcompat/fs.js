...
define([
    ...
    //  then the application can decide to implement down data source provider or not.
    //  IDE currently does not need more than 1 FS.

    function mountByFSID(fsid) {
        var wfsMount = workspaceService.getFileSystemMount();
        if (fsid !== wfsMount.wfsId) {
            throw new Error('unavailable fsid ' + fsid);
        }
        if (!privates.mounts[fsid]) {
            privates.mounts[fsid] = new CompatibleMount(wfsMount);
        }
